...
const SCRIPT_URL_INTERNA =
  'https://script.google.com/macros/s/AKfycbwA3VIQxj-Xa_Rl907ON-jZJKD3MFmiBKg4JT4_DIT-gdNEJMfhQEICwlS8Y6rOSF1e/exec'; // EXEMPLO: SUBSTITUA!
...
const SCRIPT_URL_CARREGAR_INTERNA =
  'https://script.google.com/macros/s/AKfycbzwbNHEWGZiraZDQWpfzb6qMHUTnSMy_bC6naTppcLn7hWHKnpXxaHBgjwhoB9jtIk3/exec'; // WebApp para carregar última inspeção (Interna)
...
function createFieldHTML(fieldConfig, currentValue) {
    ...
    switch (fieldConfig.type) {
        ...
        case 'date':
        case 'time':
            inputHTML = `<input type="${fieldConfig.type}" id="${fieldId}" name="${fieldConfig.name}" data-field-name="${fieldConfig.name}" value="${value}" ${required} ${readonly} placeholder="${placeholder}" ${fieldConfig.digits ? `maxlength="${fieldConfig.digits}"` : ''} onchange="handleFieldChange('${fieldConfig.name}', this.value)">`;
            break;
        case 'textarea': {
            const isDescricaoAnormalidade = /anormalidade/i.test(fieldConfig.label || '');
        ...
        case 'range': {
            ...
            const isNivelOleoGovernor   = labelLower.includes('óleo governor') || labelLower.includes('oleo governor');
            const isNivelCarter         = labelLower.includes('nível do cárter') || labelLower.includes('nivel do carter');
            const isTanqueExpansao      = labelLower.includes('tanque de expansão') || labelLower.includes('tanque de expansao');
            const isNivelMancalAcoplado = labelLower.includes('mancal acoplado');
            const isNivelMancalNaoAcoplado = labelLower.includes('mancal não acoplado') || labelLower.includes('mancal nao acoplado');
            const isNivelCarterSep      = labelLower.includes('nível cárter') || labelLower.includes('nivel carter');
            ...
            if (useIntegrator) {
                // Define step especial: 1 cm para cárter, 10% para demais
                const step = isNivelCarter ? 1 : 10;
                inputHTML = `
                ...
                `;
                // Evita exibir unidade duplicada no label
                unitHTML = '';
            } else {
...
function initializeAutomaticFields(windowId) {
    ...
    // Preenche DATA do dia automaticamente (não exibe nem altera hora aqui)
    ...
    // Preenche nomes sugeridos de operador/supervisor
...
function handleFormSubmit(event) {
    ...
    if (currentWindowId === 'dados-iniciais') {
        // Garante que a hora inicial seja registrada no momento do SALVAR dos dados iniciais
    ...
    // Após salvar Dados Iniciais, pergunta se deseja carregar a última inspeção da planilha
    if (currentWindowId === 'dados-iniciais') {
        setTimeout(() => {
...
function handleReportSubmit() {
    ...
    // Bloqueia o botão e muda o texto para indicar envio em progresso
    if (submitBtn) {
    ...
    // Registra a hora final no momento do envio do RELATÓRIO COMPLETO
    ...
    sendReportToScript(formType, dataToSend)
        ...
        .catch(err => {
            ...
            // Em caso de erro, reabilita o botão de envio
            if (submitBtn) {
...
/**
 * Busca a última linha da planilha de INSPEÇÃO INTERNA via Apps Script
 * e aplica seus valores nas janelas (exceto Dados Iniciais).
 */
async function carregarUltimaInspecaoInterna() {
    try {
        ...
        alert('Dados da inspeção anterior carregados com sucesso. Revise e ajuste antes de enviar o relatório.');
    } catch (e) {
        if (typeof hideSpinner === 'function') {
            hideSpinner();
        }
 ...
 * Usa o nome dos campos (headers da planilha) para preencher os campos de cada janela.
 */
function aplicarUltimaInspecaoInterna(rowData) {
    if (!rowData) return;

    const camposDadosIniciais = ['hora_inicial','hora_final','data','operador','supervisor','turma','turno','assinatura'];
...
// ============ INTEGRADORES ESPECIAIS E DITADO POR VOZ ============

/**
 * Ajusta o valor dos campos com integrador (+ / -) e atualiza o texto exibido.
 */
...
/**
 * Usa a Web Speech API (quando disponível) para ditar texto no campo de descrição
 * de anormalidade. Compatível com Chrome / Edge (webkitSpeechRecognition).
 */
